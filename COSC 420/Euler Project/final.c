...
void all_color_combinations(struct cubeletColors all_color_choices[], int pos, int n, struct cubeletColors color[], /**/int cube[][8], int permutes, int *counter){
    const int NUM_OF_COLORS = 2;
    ...
    if(pos < (n - 1)){
        for(i = 0; i < ncubed; i++) {
            ...
            all_color_combinations(all_color_choices, pos + 1, n, color, cube, permutes, counter);
        }
        ...
        for(i = 0; i < ncubed; i++) {
            ...
            for(i = 0; i < permutes; i++){
              for(j = i+1; j < permutes; j++){
                ...
                if(comparePermutations(color, cube, i, j) == 1){
...
int main(int argc, char** argv){
  ...
  if(rank == 0){
    ...
    all_color_combinations(c, 0, 8, color_choices, cube, numPermutes, &result);
