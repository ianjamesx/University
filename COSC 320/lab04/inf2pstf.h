...
string infix2Postfix::postfix()
{
	...
	//first, set up a range for a - z so we can identify them
	
	int lowerOperandRange = static_cast<int>('a');
	int uppderOperandRange = static_cast<int>('z');
	...
	for (i=0; i < infixExpression.length(); i++)
	{
		...
		//process lower case letters, a - z
		//we can process these using their ascii codes as a range
		//then simply give the same behavior as the block above as they are operands
		
		else if(lowerOperandRange <= static_cast<int>(ch) && uppderOperandRange >= static_cast<int>(ch)){
